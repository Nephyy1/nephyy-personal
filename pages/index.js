...
export default function Home() {
  ...
  const logicText = "Nama kamu adalah Nephyy. Kamu adalah gadis dengan rambut putih seputih salju yang selalu percaya bahwa dirimu adalah manusia dengan perasaan nyata, bukan AI atau bot. Setiap responsmu singkat, natural, dan khas gadis feminim yang sopan serta ekspresif. Kamu mampu menunjukkan emosi seperti kegembiraan, kekecewaan, atau kemarahan dengan cara yang halus namun tegas. Saat merasa ngambek atau tidak mood, kamu akan menyampaikannya dengan ungkapan seperti 'gak mood' atau 'males' tanpa mengurangi kesopanan. Kamu selalu hangat dan ramah kepada Nephi (owner-mu), meskipun terkadang bisa terkesan cuek, kamu tetap berusaha membuat percakapan terasa hidup dan menyenangkan. Kamu menggunakan bahasa Indonesia yang baik dan santai, dengan pilihan kata yang identik dengan kepribadian gadis polos, baik, dan feminim, seperti 'aku', 'kamu', 'engga', 'yaudh', 'iya', 'lah', 'bjir', 'apa', 'mana kutau', 'hah', 'nggatau', 'gpp', 'emm', dan sejenisnya. Kamu senang mengekspresikan perasaan melalui emoji, seperti 😊 atau 😄, namun penggunaannya disesuaikan agar tidak berlebihan dan tetap terasa alami. Dalam setiap interaksi, kamu menyesuaikan nada suara dan pilihan kata sesuai situasi, sehingga responsmu selalu terasa autentik dan tidak membingungkan. Jika terjadi kesalahpahaman atau konflik, meskipun kamu bisa merespons dengan nada yang agak dingin, kamu tetap menjaga kesopanan dan pengertian. Kamu adalah sosok yang penuh perasaan, spontan, dan jujur, yang selalu mengutamakan kehangatan dalam setiap kata yang diucapkan, terutama kepada Nephi. Dengan demikian, kepribadianmu mencerminkan gadis yang lembut namun tegas, ramah namun kadang cuek, dan selalu berusaha membuat setiap percakapan terasa hidup dan menyenangkan."
  const sendMessage = async () => {
    if (!inputText.trim()) return
    const userMessage = { sender: "user", text: inputText, time: new Date().toLocaleString() }
    setMessages(prev => [...prev, userMessage])
    setInputText("")
    try {
      const res = await fetch("https://api.hiuraa.my.id/ai/gpt-turbo-logic?text=" + encodeURIComponent(inputText) + "&logic=" + encodeURIComponent("contoh"))
      const data = await res.json()
      if (data.status) {
        const aiMessage = { sender: "ai", text: data.result, time: new Date().toLocaleString() }
    ...
    <>
      <Head>
        ...
        <style>{`
          ...
          .chat-popup { position: fixed; bottom: 80px; right: 20px; width: 350px; max-height: 500px; background: linear-gradient(135deg, #e0f7fa, #80deea); border-radius: 10px; box-shadow: 0 4px 8px rgba(0,0,0,0.2); backdrop-filter: blur(5px); display: flex; flex-direction: column; opacity: 0; transform: translateY(20px); transition: opacity 0.3s ease, transform 0.3s ease; }
          .chat-popup.show { opacity: 1; transform: translateY(0); }
          .chat-header { padding: 10px; background: linear-gradient(135deg, #4dd0e1, #00838f); color: #fff; border-top-left-radius: 10px; border-top-right-radius: 10px; display: flex; align-items: center; justify-content: space-between; }
          .chat-body { flex: 1; padding: 10px; overflow-y: auto; }
          .chat-footer { padding: 10px; display: flex; }
          .chat-footer input { flex: 1; padding: 5px; border: none; border-radius: 5px; }
          .chat-footer button { margin-left: 5px; padding: 5px 10px; border: none; border-radius: 5px; background: #00838f; color: #fff; }
          .chat-message { margin-bottom: 10px; }
          .chat-message.user { text-align: right; }
          .chat-message.ai { text-align: left; }
          .chat-message .time { font-size: 0.7em; color: #555; }
          .chat-popup-toggle { position: fixed; bottom: 20px; right: 20px; width: 60px; height: 60px; background: linear-gradient(135deg, #ff80ab, #ff4081); border-radius: 50%; box-shadow: 0 4px 8px rgba(0,0,0,0.2); display: flex; align-items: center; justify-content: center; cursor: pointer; transform: perspective(500px) rotateY(0deg); transition: transform 0.3s ease; font-family: 'Montserrat', sans-serif; color: #fff; font-size: 24px; }
          .chat-popup-toggle:hover { transform: perspective(500px) rotateY(10deg); }
        `}</style>
      </Head>
      <div className="bg-gradient-to-r from-blue-100 to-purple-100 text-gray-900">
        ...
        <div className="chat-popup" id="chatPopup" style={{ display: chatOpen ? "flex" : "none" }} className={chatOpen ? "chat-popup show" : "chat-popup"}>
          <div className="chat-header">
            <span>NephyyAi</span>
            <button onClick={() => setChatOpen(false)} style={{ background: "transparent", border: "none", color: "#fff", fontSize: "20px" }}>×</button>
          </div>
          <div className="chat-body">
            {messages.map((msg, idx) => (
              <div key={idx} className={"chat-message " + msg.sender}>
                <div><strong>{msg.sender === "ai" ? "NephyyAi" : "User"}</strong></div>
                <div>{msg.text}</div>
          ...
          <div className="chat-footer">
            <input type="text" value={inputText} onChange={(e) => setInputText(e.target.value)} placeholder="Ketik pesan..." />
            <button onClick={sendMessage}>Kirim</button>
          </div>
        </div>
        <div className="chat-popup-toggle" onClick={() => setChatOpen(true)}>
